...
export async function handleChatCore({
  ...
  if (!stream) {
    try {
      ...
      // Non-streaming 401/403 reactive refresh: mirrors the streaming branch
      // (~3912). The streaming 401/403 refresh only runs inside `if (stream)`,
      // so non-streaming requests never refreshed credentials on auth failures.
      // This restores parity: refresh via the executor, then re-run the leg once
      // with the refreshed credentials.
      if (
        legResult.kind === "error" &&
        (legResult.result.status === HTTP_STATUS.UNAUTHORIZED ||
          legResult.result.status === HTTP_STATUS.FORBIDDEN) &&
        !(await shouldIsolateProbeFailures())
      ) {
        const attemptedRefreshToken =
          typeof credentials?.refreshToken === "string" ? credentials.refreshToken : null;
        ...
        if (newCredentials?.accessToken || newCredentials?.copilotToken) {
          log?.info?.("TOKEN", `${provider?.toUpperCase()} | refreshed (non-streaming)`);
          if (!persistFnRan) {
            Object.assign(credentials, newCredentials);
            if (onCredentialsRefreshed) {
              await onCredentialsRefreshed(newCredentials);
          ...
          // Retry the leg once with refreshed credentials.
          try {
            legResult = await runNonStreamingProviderLeg({
              phase: "initial",
              sourceBody: (body || {}) as Record<string, unknown>,
              expectedConnectionId: managedLease
                ? String(getCurrentConnectionId() || connectionId || "") || undefined
            ...
            log?.error?.(
              "TOKEN",
              `${provider?.toUpperCase()} | retry after refresh failed (non-streaming): ${sanitizeErrorMessage(retryErr)}`
            );
          }
        } else {
          log?.warn?.("TOKEN", `${provider?.toUpperCase()} | refresh failed (non-streaming)`);
          if (isUnrecoverableRefreshError(newCredentials) && onCredentialsRefreshed) {
            let alreadyRotated = false;
            if (typeof connectionId === "string" && connectionId && attemptedRefreshToken) {
              try {
      ...
      if (legResult.kind === "error") {
        ...
        // Non-streaming abort normalization: mirrors the streaming branch.
        // Raw abort reasons (AbortError, "request_signal_aborted", bare strings)
        // must map to 499 "Request aborted", not 502.
        const isAborted = isLocalStreamLifecycleError(err.originalError ?? err.error);
        if (isAborted) {
          const abortedResult = {
            ...err,
            status: 499,
          ...
          persistAttemptLogs({
            ...
            error: "Request aborted",
            providerRequest: finalBody || translatedBody,
            // Omit clientResponse for aborts: client already disconnected.
        ...
        // Non-streaming rate-limit sync: mirrors the streaming branch (~4543).
        // A 429 must drain the runtime reservoir so the body-derived retry-after
        // is honored; without this the limiter never sees non-streaming 429s.
        if (!isNetworkThrow && err.status === 429 && connectionId) {
          try {
            if (err.response?.headers) {
              updateFromHeaders(provider, connectionId, err.response.headers, err.status, model);
            }
            // Reconstruct a body for retry-after parsing from the error message
            // (the leg consumed the raw body while building err.error).
            updateFromResponseBody(
              provider,
              connectionId,
            ...
            // Rate-limit bookkeeping must never fail the request.
          }
        }
        // Non-streaming per-model quota lock: mirrors the streaming branch
        // (~4388). A 402/quota-exhausted failure locks only the affected model
        // for providers with per-model quotas (e.g. gemini), keeping the
        // connection active for other models.
        if (
          !isNetworkThrow &&
          (err.status === 402 || err.errorType === PROVIDER_ERROR_TYPES.QUOTA_EXHAUSTED) &&
          connectionId
        ) {
          const quotaCooldownMs = COOLDOWN_MS.rateLimit;
          if (
            lockModelIfPerModelQuota(
              provider,
              connectionId,
            ...
            console.warn(
              `[provider] Node ${connectionId} quota exhausted (${err.status}) for ${model} - ${Math.ceil(quotaCooldownMs / 1000)}s (non-streaming, connection stays active)`
            );
          }
        }
        trackPendingRequest(model, provider, connectionId, false);
      ...
      // The streaming branch builds claudePromptCacheLogMeta at ~3747; the
      // non-streaming leg never runs that branch, so build it here for parity —
      // every persistAttemptLogs below expects it for Claude prompt-cache metadata.
      if (!claudePromptCacheLogMeta) {
        claudePromptCacheLogMeta = buildClaudePromptCacheLogMeta(
          targetFormat,
          finalBody,
          providerHeaders,
