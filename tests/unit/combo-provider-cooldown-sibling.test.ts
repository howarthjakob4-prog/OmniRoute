...
test("source guard: executeTargetAttempt skips provider cooldown for per-model-quota on 500", () => {
  // #5976 logic moved out of the monolithic open-sse/services/combo.ts into
  // open-sse/services/combo/executeTargetAttempt.ts during the combo split.
  const src = fs.readFileSync(
